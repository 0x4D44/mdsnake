...
/** A legend entry: a preset name plus any per-cell overrides. In Inc 1 presets
 *  carry no id-bearing overrides, so this is just `{ type }`; the shape is ready
 *  to gain `trigger`/`door` ids when those presets land (Inc 3). */
